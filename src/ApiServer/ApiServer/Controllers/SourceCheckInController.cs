    ...
    public class SourceCheckInController : BaseRegistryController {
        ...
        public async Task<IActionResult> CreateCheckin(
            ...
            if(source.CountMeInProvider == null) {
                Logger.LogWarning("Cannot create check-in event for source {0} because it is not linked to any Count Me In provider", source.Id);
            ...
            return Created(
                Url.Action(nameof(GetCheckin), new { eventId }),
                new CreateCheckInOutput(
                    $"https://{SelfHostDomain}",
                    $"{eventId:D}",
                    $"https://{SelfLinkDomain}/cmi/{Guid.NewGuid():D}/{Guid.NewGuid():D}"
                )
            );
        }
